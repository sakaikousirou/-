...
// --- 全235枚のカードマスター自動生成ロジック (省略せずそのまま) ---
...
function resolveAttack(game, room, defenderCard = null) {
  ...
  if (!teamAAlive || !teamBAlive) {
    ...
    game.phase = "END";
    game.winnerTeam = winnerTeam; // 勝者限定チャット用
    ...
    return; // ゲーム終了状態のまま維持（チャットのためdeleteしない）
  }

  advanceTurn(game, room, log);
}

// ターンを次の生きているプレイヤーへ進める
function advanceTurn(game, room, log = "") {
  ...
  game.phase = "ATTACK";
  game.players[game.currentTurn].mp += 3; // ターン開始時MP回復
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (data) => {
    const { roomNumber, mode } = data; // 例: roomNumber: 1, mode: '2v2'
    ...
    // プレイヤー番号の割り振り (1v1なら1,2 / 2v2なら1,2,3,4)
    const playerNumber = Object.keys(game.players).length + 1;
    socket.join(roomName);
    game.sockets[playerNumber] = socket.id;
    game.players[playerNumber] = { 
      id: socket.id, team: (playerNumber % 2 !== 0) ? "A" : "B", // 奇数はA、偶数はB
      charKey: "suzuki", char: null, hp: 60, mp: 10, isDead: false, hand: generateHand(), ready: false 
    };
    ...
    if (Object.keys(game.players).length === maxPlayers) {
      game.phase = "SELECT";
      if (mode === "2v2") game.turnOrder = [1, 2, 3, 4]; // ターン順: A1→B1→A2→B2
  ...
  socket.on("playCard", (data) => {
    ...
    if (game.phase === "ATTACK") {
      ...
      // ターゲットの決定（UIから指定がない場合は敵チームの生きている先頭を指定）
      ...
      if (card.type === "heal") {
        ...
        game.phase = "DEFENSE"; 
        game.currentTurn = targetNum; // 防御する人にターンを移す
  ...
  socket.on("passTurn", (data) => {
    ...
    if (game.phase === "DEFENSE") {
      resolveAttack(game, data.room, null); // 防御放棄
    } else {
